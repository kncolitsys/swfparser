//////////////////////////////////////////////////////////////////////////////////////////////////
//
//  Based on Adobe file AbcPrinter.java part of swfutils.jar of Opensource Adobe FLEX project.
//	SWFparser.cfc helper java class to interpret SWF actionscript3 byte codes (abc)
//
//	Author: Pablo Schaffner Bofill
//	Date: June 21, 2012
//	Part of: swfparser.cfc (Coldfusion SWF Component)
//	License: GPL
//	
//
/////////////////////////////////////////////////////////////////////////////////////////////////
...
/**
 * This utility supports reading AS3 ABC.
 */
public class AbcInterpreter 
{
	...
	private co_class[] classes;
    private MethodInfo[] methods;
    private String[] instanceNames;
    private String indentString;
		
    public AbcInterpreter(byte[] abc, boolean showOffset, int indent, boolean showByteCode)
    {
        this.abc = abc;
        //this.out = out;
    ...
    public co_class[] getClasses()
    {
			
		String minor_version = "" + abc[offset++] + "." + abc[offset++];
		String mayor_version = "" + abc[offset++] + "." + abc[offset++];
		readIntConstantPool();
    ...
    void readMethods()
    {
        ...
        for (int i = 0; i < n; i++)
        {
			...
			m.iparams = new String[m.paramCount];
            for (int j = 0; j < m.paramCount; j++)
            {
                m.params[j] = (int)readU32();
				m.iparams[j] = multiNameConstants[m.params[j]].toString();
            }
            int nameIndex = (int)readU32();
            if (nameIndex > 0)
    ...
    {
        long n = readU32();
		classes = new co_class[(int)n];
		instanceNames = new String[(int)n];
        for (int i = 0; i < n; i++)
        {
            int start = offset;
			co_class cClass = classes[i] = new co_class();
            ...
            int init = (int)readU32(); // eat init method (1 record only)
            ...
            //out.print(name + " ");
			// add info to class array
			cClass.name = name;
			cClass.co_extends = base;
			cClass.co_implements = s;		
				
            int numTraits = (int)readU32(); // number of traits
			int n_consts = 0;
			// prepare variables array for cClass instance
			co_var[] vars = new co_var[numTraits];
			for (int j = 0; j < numTraits; j++)
            {
                //printOffset();
				int indexMultiName = (int)readU32();
                start = offset;
                s = multiNameConstants[indexMultiName].toString(); // eat trait name;
                b = abc[offset++];
				int g = 0;
                int kind = b & 0xf;
                switch (kind)
                ...
                case 0x00:	// slot
                case 0x06:	// const
					co_var cVar = vars[n_consts] = new co_var();
					n_consts+=1;
                    long slot_id = readU32();	// id
                    int type_name = (int)readU32();	// type_name index into multiname
                    int v_index = (int)readU32();	// index;
					String v_typename = multiNameConstants[type_name].toString();
                    if (v_index != 0) {
                        g = abc[offset++];	// kind
						int v_kind = g & 0xf;
						switch (v_kind)
						{
							case 0x03:	// use integer array for value
								cVar.name=""+s;
								cVar.type=""+v_typename;
								cVar.value=""+intConstants[v_index];
								break;
							case 0x04:	// use uinteger array
								cVar.name=""+s;
								cVar.type=""+v_typename;
								cVar.value=""+uintConstants[v_index];
								break;
							case 0x01:	// use string array
								cVar.name=""+s;
								cVar.type=""+v_typename;
								cVar.value=""+stringConstants[v_index];
								break;
							case 0x0B:	// value: true
								cVar.name=""+s;
								cVar.type=""+v_typename;
								cVar.value="true";
								break;
							...
							case 0x00:
								cVar.name=""+s;
								cVar.type=""+v_typename;
								cVar.value="undefined";
								break;
							case 0x08:	// namespace array
							...
							case 0x05:	// constant private namespace
								cVar.name=""+s;
								cVar.type=""+v_typename;
								cVar.value=""+namespaceConstants[v_index];
								break;
						}
					} else {
						cVar.name=""+s;
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            int numTraits = (int)readU32(); // number of traits
			co_var[] vars = new co_var[numTraits];	// to read Init Method's vars
			int n_consts = 0;
            for (int j = 0; j < numTraits; j++)
            {
                //printOffset();
	...
	void assignBodies()
	{
		// call it after readMethodBodies() !
		// completes the 'classes' object variable with remaining information.
		// var classes...
		for (int i=0; i < classes.length; i++)
		{
			// for each class..
			// count number of methods inside this class first.. (to create array)
			int numMethods = 0;
			for (int c=0; c < methods.length; c++)
			{
				MethodInfo mi = methods[c];
				if (mi.className==classes[i].name) {
					// this method is of the current class
					numMethods = numMethods+1; 
				}
			}
			// loop within methods[] object searching for class name.. and assign...
			co_method[] tmp_methods = new co_method[(int)numMethods];
			int met_pos = 0;
			for (int c=0; c < methods.length; c++)
			{
				MethodInfo mi = methods[c];
				if (mi.className==classes[i].name) {
					// this method is of the current class... add it to the class.
					co_method the_method = tmp_methods[met_pos] = new co_method();
					met_pos = met_pos+1;
					the_method.class_name = mi.className;
					the_method.name = mi.name;
					the_method.returntype = multiNameConstants[mi.returnType].toString();
					the_method.iparams = mi.iparams;
					the_method.content = mi.content;
					the_method.original_index = c;
					the_method.vars = mi.vars;
				}
			}
			// assign to original class obj.
			classes[i].i_methods = tmp_methods;
		//
		}
	}
		
    void readMethodBodies()
    {
        long n = readU32();
		...
		for (int i = 0; i < n; i++)
        {
            ...
            while (offset < stopAt)
            {
                ...
                if (opcode == OP_label || labels.hasLabelAt(offset - 1)) 
                {
					...
					function_code += "<opcode label=\""+s+"\" name=\""+opNames[opcode].trim()+"\">";
                    while (s.length() < 4)
                        s += " ";					
                } else {
                    s = "    ";
					function_code += "<opcode name=\""+opNames[opcode].trim()+"\">";
				}
                ...
                {
                case OP_debugfile:
                	stmp = stringConstants[(int)readU32()].replaceAll("\n","\\n").replaceAll("\t","\\t");
					function_code += "<code type=\"debug\" value=\""+stmp+"\"/>";
					s += '"' + stmp + '"';
                	break;
                case OP_pushstring:
                    stmp = stringConstants[(int)readU32()].replaceAll("\n","\\n").replaceAll("\t","\\t");
					function_code += "<code type=\"string\" value=\""+stmp+"\"/>";
					s += '"' + stmp + '"';
                    break;
                case OP_pushnamespace:
                    stmp = namespaceConstants[(int)readU32()];
                    function_code += "<code type=\"namespace\" value=\""+stmp+"\"/>";
					s += stmp;
					break;
                case OP_pushint:
                    int k = intConstants[(int)readU32()];
                    stmp = k + "\t// 0x" + Integer.toHexString(k);
					function_code += "<code type=\"int\" value=\""+stmp+"\"/>";
					s += stmp;
                    break;
                case OP_pushuint:
                    long u = uintConstants[(int)readU32()];
                    stmp = u + "\t// 0x" + Long.toHexString(u);
					function_code += "<code type=\"uint\" value=\" 0x"+Long.toHexString(u)+"\"/>";
					s += stmp;
                    break;
                case OP_pushdouble:
                    int f = (int)readU32();
                    stmp = "floatConstant" + f;
					function_code += "<code type=\"double\" value=\""+f+"\"/>";
					s += stmp;
                    break;
                case OP_getsuper: 
                case OP_setsuper: 
                ...
                case OP_getdescendants:
                    stmp = ""+multiNameConstants[(int)readU32()];
					function_code += "<code type=\"descendant\" value=\""+stmp+"\"/>";
					s += stmp;
                    break;
                ...
                case OP_callpropvoid:
                    stmp = ""+multiNameConstants[(int)readU32()];
					stmpb = ""+readU32();
					function_code += "<code type=\"property_void\" property=\""+stmp+"\" value=\""+stmpb+"\"/>";
					stmp += " (" + stmpb + ")";
					s += stmp;
                    break;
                case OP_newfunction:
                    int method_id = (int)readU32();
                    stmp = ""+methods[method_id].name;
					function_code += "<code type=\"newfunction\" value=\""+stmp+"\"/>";
					s += stmp;
                    // abc.methods[method_id].anon = true  (do later?)
                    break;
                case OP_callstatic:
                    stmp = ""+methods[(int)readU32()].name;
					stmpb = ""+readU32();
					function_code += "<code type=\"callstatic\" name=\""+stmp+"\" value=\""+stmpb+"\"/>";
                    stmp += " (" + stmpb + ")";
					s += stmp;
                    break;
                case OP_newclass: 
                    stmp = ""+instanceNames[(int)readU32()];
					function_code += "<code type=\"newclass\" value=\""+stmp+"\"/>";
					s += stmp;
                    break;
                case OP_lookupswitch:
                    ...
                    stmp = "default:" + labels.getLabelAt(target); // target + "("+(target-pos)+")"
                    stmp += " maxcase:" + Integer.toString(maxindex);
					function_code += "<code type=\"switch\" default=\""+labels.getLabelAt(target)+"\" maxcase=\""+Integer.toString(maxindex)+"\">";
                    for (int m = 0; m <= maxindex; m++) 
                    {
                        target = pos + readS24();
						function_code += "<case label=\""+labels.getLabelAt(target)+"\"/>";
                        stmp += " " + labels.getLabelAt(target); // target + "("+(target-pos)+")"
                    }
					function_code += "</code>";
					s += stmp;
                    break;
                ...
                case OP_ifstricteq:	case OP_ifstrictne:
                    ...
                    //s += target + " ("+offset+")"
                    stmp = ""+labels.getLabelAt(targ);
					function_code += "<code type=\"if\" value=\""+stmp+"\"/>";
                ...
                case OP_newcatch:
                    stmp = ""+readU32();
					function_code += "<code type=\"new_catch\" value=\""+stmp+"\"/>";
					s += stmp;
                    break;
                case OP_debug:
                    stmp = ""+Integer.toString(abc[offset++] & 0xFF); 
                    stmp += " " + readU32();
                    stmp += " " + Integer.toString(abc[offset++] & 0xFF);
                    stmp += " " + readU32();
					s += stmp;
                ...
                case OP_newobject:
                    stmp = "" + readU32();
					function_code += "<code type=\"new_object\" value=\""+stmp+"\"/>";
					s += "{" + stmp + "}";
                    break;
                case OP_newarray:
                    stmp = "" + readU32();
					function_code += "<code type=\"new_array\" value=\""+stmp+"\"/>";
					s += "["+stmp+"]";
                    break;
                case OP_call:
                case OP_construct:
                case OP_constructsuper:
                    stmp = "" + readU32();
					function_code += "<code type=\"construct_super\" value=\""+stmp+"\"/>";
					s += "("+stmp+")";
                    break;
                case OP_pushbyte:
                case OP_getscopeobject:
                    stmp = ""+abc[offset++];
					function_code += "<code type=\"get_scope_object\" value=\""+stmp+"\"/>";
					s += stmp;
                    break;
                case OP_hasnext2:
                    stmp = ""+readU32();
 					stmpb = ""+readU32();
					function_code += "<code type=\"hasnext\" first=\""+stmp+"\" second=\""+stmpb+"\"/>";
					s += stmp+" "+stmpb;
                default:
			...
			//PSB :: OUTPUT TAGGED CONTENT
			//printOffset();
			mi.content = function_code;	//psb: assign function content to method_content
				//out.println(function_open);
			//printOffset();
				//out.println(function_params);
			//printOffset();
			//out.println(function_code);
			//printOffset();
				//out.println(function_close);
            int exCount = (int)readU32();
			//printOffset();
            //out.println(exCount + " Extras");	// desc: try/catch statements.
            for (int j = 0; j < exCount; j++)
            {
                ...
                //out.print(multiNameConstants[nameIndex] + " ");		//ex: catch (multiNameConstant=err:type=Error)
			...
			int n_consts = 0;
            //printOffset();
            //out.println(numTraits + " Traits Entries");	// vars inside current method.
            for (int j = 0; j < numTraits; j++)
            {
                //printOffset();
                start = offset;
				...
				// s=variable que esta dentro de este metodo...
                //out.println(s);
            }
            //out.println("");
			//
	...
	class co_class {
		...
		{
			return co_implements;
		}
		public co_var[] getVars()
		{
			return vars;
		}
		public co_method[] getMethods()
		{
			return i_methods;
		}
	}
	...
	class co_method {
		...
		String name;
		String returntype;
		int original_index;	// to map it inside readMethodBodies.. (to complete opcodes, params, returntype and vars)
		String[] iparams;
		co_var[] vars;
		String content;
